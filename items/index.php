...
<body>
   <div class="container">
    ...
    if (isset($_POST["submit"])) {
        ...
        // Modify the SQL query to include the published status condition
        $sql = "SELECT * FROM `item_list` WHERE `status` = 1"; // Only get published items
        $params = [];
        ...
        if ($sth->rowCount() > 0) {
            // Output results or process as needed
            ?>
            ...
            <div class="col-lg-3 col-md-4 col-sm-12 col-12">
                ...
                <div class="dropdown mb-3">
                    <button class="btn btn-primary dropdown-toggle w-100" type="button" id="categoryDropdown" data-bs-toggle="dropdown" aria-expanded="false">
                        <?php 
                        // Display the selected category name or a default label
                        if (isset($cat['name'])) {
                            echo $cat['name'];
                        } else {
                    ...
                    <ul class="dropdown-menu w-100" aria-labelledby="categoryDropdown">
                        ...
                        ?>
                        <li>
                            <a href="<?= base_url.'?page=items&cid='.$row['id'] ?>" class="dropdown-item<?= (isset($_GET['cid']) && $_GET['cid'] == $row['id']) ? ' active': '' ?>">
                                <?= $row['name'] ?>
                            </a>
                        </li>
                        <?php endwhile; ?>
