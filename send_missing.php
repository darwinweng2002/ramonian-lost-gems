...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO missing_items (user_id, title, description, last_seen_location, time_missing, contact, category_id, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("isssss", $userId, $title, $description, $lastSeenLocation, $timeMissing, $contact, $category_id $status);
    $stmt->execute();
